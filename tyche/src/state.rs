...
impl PebbleHashChain {
    ...
    /// Asynchronous version that creates a hash chain directly from a known seed (no derivation).
    /// Used when the seed comes from on-chain commitment metadata.
    pub async fn from_seed_async(
        seed: [u8; 32],
        chain_length: u64,
        sample_interval: u64,
    ) -> Result<Self> {
        ...
        let hash_chain = spawn_blocking(move || Self::new(seed, chain_length, sample_interval))
            .await
