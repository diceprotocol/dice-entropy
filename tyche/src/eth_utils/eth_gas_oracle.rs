...
// Untychetely this logic is not configurable in ethers.rs.
//
    ...
    M::Error: 'static,
{
    async fn fetch(&self) -> Result<U256> {
        // For legacy transactions, eth_gasPrice() returns the current block's
        // gas price. But by the time our tx is submitted, the next block's base
        // fee may have risen slightly — causing "max fee per gas less than block
        // base fee" rejections. Add a 50% buffer to ensure first-attempt inclusion on volatile L2 gas.
        let raw_price = self
            .provider
            .get_gas_price()
            .await
            .map_err(|err| GasOracleError::ProviderError(Box::new(err)))?;
        Ok(raw_price * 150 / 100)
    }
