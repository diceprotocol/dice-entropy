use {
    crate::{
        chain::ethereum::{DiceRandomEvents, InstrumentedSignableDiceContract, RevealedFilter},
...
pub struct SubmitTxResult {
    ...
    pub revealed_event: RevealedFilter,
}
...
pub async fn submit_tx_with_backoff<T: Middleware + NonceManaged + 'static>(
    ...
    let revealed_event: RevealedFilter = {
        let mut found_event = None;
        for log in &receipt.logs {
            if let Ok(DiceRandomEvents::RevealedFilter(decoded_event)) =
                DiceRandomEvents::decode_log(&log.clone().into())
            {
...
pub async fn submit_tx<T: Middleware + NonceManaged + 'static>(
    ...
    let pending_tx = client
        ...
        .await
        .map_err(|e| {
            // "nonce too low" means the in-memory NonceManager is stale.
            // Reset it so the next retry queries the chain for the true nonce.
            let msg = format!("{e:?}");
            if msg.contains("nonce too low") || msg.contains("nonce has already been used") {
                tracing::warn!(
                    "Nonce too low error detected. Resetting nonce manager. Error: {}",
                    msg
                );
                client.reset();
            }
    ...
    // Capture the tx hash so we can query the receipt manually if the
    // PendingTransaction future resolves early or times out.
    ...
    // Handle both timeout and None cases by checking the chain for the actual receipt.
    // On Robinhood Chain, PendingTransaction can time out or resolve None even when
    // the tx was successfully mined (sequencer delay + receipt indexing lag).
    let receipt = match pending_receipt {
        Ok(Ok(Some(r))) => r,
        Ok(Ok(None)) => {
            tracing::warn!(
                "Pending tx {:?} resolved None. Querying chain for receipt.",
                tx_hash
            );
            match client.get_transaction_receipt(tx_hash).await {
                Ok(Some(r)) => {
                    tracing::info!(
                        "Tx {:?} was mined (block {:?}) despite None pending. Returning receipt.",
                        tx_hash,
                        r.block_number
                    );
                    client.reset();
                ...
                _ => {
                    ...
                    return Err(backoff::Error::transient(
                        SubmitTxError::ConfirmationTimeout(transaction.clone()),
                    ));
                }
            }
        }
        Ok(Err(e)) => {
            return Err(backoff::Error::transient(
                SubmitTxError::ConfirmationError(transaction.clone(), e),
            ))
        }
        Err(_) => {
            // Timeout. Query chain before retrying.
            tracing::warn!(
                "Pending tx {:?} timed out after {}s. Querying chain for receipt.",
                tx_hash,
            ...
            match client.get_transaction_receipt(tx_hash).await {
                Ok(Some(r)) => {
                    tracing::info!(
                        "Tx {:?} was mined (block {:?}) despite timeout. Returning receipt.",
                        tx_hash,
                        r.block_number
                    );
                    client.reset();
