use {
    ...
    crate::{
        api::ChainId, chain::ethereum::InstrumentedDiceContract,
...
pub async fn track_provider(
    chain_id: ChainId,
    contract: InstrumentedDiceContract,
...
/// tracks the accrued dice fees on the given chain
#[tracing::instrument(skip_all)]
pub async fn track_accrued_dice_fees(
    chain_id: ChainId,
    contract: InstrumentedDiceContract,
    metrics: Arc<KeeperMetrics>,
) -> Result<()> {
    let accrued_dice_fees = contract.get_accrued_treasury_fees().call().await?;

    // The f64 conversion is made to be able to serve metrics with the constraints of Prometheus.
    // The fee is in wei, so we divide by 1e18 to convert it to eth.
    let accrued_dice_fees = accrued_dice_fees.to_f64().ok_or_else(|| {
        anyhow!(
            "Failed to convert accrued_dice_fees value {:?} to f64",
            accrued_dice_fees
        )
    })? / 1e18;
