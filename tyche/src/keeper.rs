use {
    crate::{
        ...
        chain::ethereum::{InstrumentedDiceContract, InstrumentedSignableDiceContract},
        ...
        keeper::{
            ...
            track::{
                track_accrued_dice_fees, track_balance, track_block_timestamp_lag, track_provider,
            },
        },
    },
    anyhow,
...
pub(crate) mod mined_tx;
pub(crate) mod process_event;
pub(crate) mod track;
pub(crate) mod ws_block;
...
pub async fn run_keeper_threads(
    ...
    let contract = Arc::new(InstrumentedSignableDiceContract::from_config(
        &chain_eth_config,
        &keeper_private_key,
    ...
    let reveal_lock = Arc::new(tokio::sync::Mutex::new(()));
    ...
    // Use WebSocket if configured, otherwise fall back to HTTP polling.
    if let Some(ws_url) = chain_eth_config.ws_rpc_addr.clone() {
        tracing::info!(
            "WebSocket-only mode (no HTTP poller): {}",
            crate::keeper::ws_block::mask_ws_url(&ws_url)
        );
        let ws_tx = tx.clone();
        let ws_chain_state = chain_state.clone();
        let ws_contract_addr = format!("{:x}", chain_eth_config.contract_addr);
        let mtx_ws_url = chain_eth_config
            .mined_tx_ws_rpc_addr
            .clone()
            .unwrap_or_else(|| ws_url.clone());
        let ws_contract_addr = format!("{:x}", chain_eth_config.contract_addr);
        spawn(
            async move {
                crate::keeper::ws_block::watch_blocks_ws(
                    ws_chain_state,
                    latest_safe_block,
                    ws_tx,
                    &ws_url,
        ...
        // NOTE: minedTransactions sidecar DISABLED.
        // eth_getLogs works fine on Robinhood Chain (~150ms delay) on valid txs.
        // The sidecar caused two problems:
        //   1. HTTP 429 rate limiting (shares Alchemy key with WS queries)
        //   2. Sent BlockRange{from:1, to:current} which made the processor
        //      crawl 13.6M blocks instead of just recent ones.
        // The heartbeat loop in ws_block.rs handles detection reliably.
        // Re-enable only if log indexing breaks again on VALID transactions.
        /*
        let mtx_tx = tx.clone();
        let mtx_contract_addr = format!("{:x}", chain_eth_config.contract_addr);
        let mtx_keeper_addr = format!("{:x}", keeper_address);
        let mtx_reveal_delay = chain_state.reveal_delay_blocks;
        tracing::info!(
            "minedTransactions fast-path using WS: {}",
            crate::keeper::ws_block::mask_ws_url(&mtx_ws_url)
        );
        spawn(
            async move {
                crate::keeper::mined_tx::watch_mined_transactions(
                    &mtx_ws_url,
                    &mtx_contract_addr,
                    &mtx_keeper_addr,
                    mtx_tx,
        ...
        */
        ...
        // No WS configured — use HTTP polling as the sole detection mechanism.
        tracing::info!("HTTP polling mode (no WebSocket configured)");
    ...
    if let Some(fee_manager_private_key) = fee_manager_private_key.clone() {
        let contract_as_fee_manager = Arc::new(InstrumentedSignableDiceContract::from_config(
            &chain_eth_config,
    ...
    spawn(
        async move {
            ...
            let contract = match InstrumentedDiceContract::from_config(
                &chain_config,
                chain_id.clone(),
                rpc_metrics,
            ) {
                Ok(r) => r,
                Err(e) => {
                    tracing::error!("Error while connecting to Dice contract. error: {:?}", e);
                    return;
                }
            };
            ...
            loop {
                ...
                if let Err(e) = track_accrued_dice_fees(
                    chain_id.clone(),
                    contract.clone(),
                    keeper_metrics.clone(),
                )
                .await
                {
                    tracing::error!("Error tracking accrued dice fees: {:?}", e);
                    continue;
                }
