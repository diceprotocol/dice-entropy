use {
    crate::{
        ...
        chain::ethereum::SignableDiceContract,
...
pub async fn register_provider_from_config(
    ...
    let provider_address = private_key_string.parse::<LocalWallet>()?.address();

    // Initialize a Provider to interface with the EVM contract.
    let contract =
        Arc::new(SignableDiceContract::from_config(chain_config, &private_key_string).await?);
    ...
    let call = contract.register_for(
        provider_address,
        fee_in_wei,
        commitment,
