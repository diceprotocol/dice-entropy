use {
    crate::{
        ...
        chain::ethereum::InstrumentedDiceContract,
...
pub async fn run_api(
    ...
    tags(
    (name = "tyche", description = "Random number service for the Dice Protocol")
    ),
    servers(
        (url = "https://tyche.diceprotocol.world", description = "Default Provider for mainnet chains"),
        (url = "https://tyche-staging.diceprotocol.world", description = "Default Provider for testnet chains")
    )
    )]
    struct ApiDoc;
...
async fn setup_chain_state(
    ...
    let contract = Arc::new(InstrumentedDiceContract::from_config(
        chain_config,
        chain_id.clone(),
        rpc_metrics.clone(),
    )?);
    ...
    // Security-hardened mode: seed comes from local config, not on-chain storage.
    // If metadata is empty, build it from the local provider secret.
    let latest_metadata = if provider_info.commitment_metadata.is_empty() {
        let seed_bytes = hex::decode(secret.trim_start_matches("0x"))
            .map_err(|e| anyhow!("Failed to decode provider secret: {}", e))?;
        let mut seed = [0u8; 32];
        seed.copy_from_slice(&seed_bytes[..32]);

        CommitmentMetadata {
            seed,
            chain_length: provider_info.end_sequence_number
                - provider_info.original_commitment_sequence_number,
        }
    } else {
        bincode::deserialize::<CommitmentMetadata>(&provider_info.commitment_metadata).map_err(
            |e| {
                anyhow!(
    ...
    for commitment in &provider_commitments {
        ...
        let pebble_hash_chain = PebbleHashChain::from_seed_async(
            commitment.seed,
    ...
    // Build fallback read-only contract if fallback_rpc_addr is configured.
    let fallback_contract = if let Some(ref fallback_url) = chain_config.fallback_rpc_addr {
        if !fallback_url.is_empty() {
            tracing::info!("Configuring fallback RPC: {}", fallback_url);
            let mut fallback_config = chain_config.clone();
            fallback_config.geth_rpc_addr = fallback_url.clone();
            fallback_config.fallback_rpc_addr = None;
            fallback_config.ws_rpc_addr = None;
            fallback_config.mined_tx_ws_rpc_addr = None;
            match InstrumentedDiceContract::from_config(
                &fallback_config,
                chain_id.clone(),
                rpc_metrics.clone(),
            ) {
                Ok(fc) => Some(Arc::new(fc) as Arc<dyn EntropyReader>),
                Err(e) => {
                    tracing::warn!("Failed to create fallback contract: {}. Continuing without fallback.", e);
                    None
                }
            }
        } else {
