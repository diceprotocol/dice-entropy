use {
    crate::{
        ...
        chain::ethereum::{ProviderInfo, SignableDiceContract},
...
async fn setup_chain_provider(
    ...
    let contract = Arc::new(SignableDiceContract::from_config(chain_config, &private_key).await?);
...
async fn sync_uri(
    contract: &Arc<SignableDiceContract>,
