use {
    crate::{
        chain::ethereum::{RevealedFilter, SignableDiceContract},
...
pub async fn generate(opts: &GenerateOptions) -> Result<()> {
    let contract = Arc::new(
        SignableDiceContract::from_config(
    ...
    while !found_request && num_retries < 10 {
        ...
        let mut event = contract.revealed_filter();
        event.filter = event
            .filter
        ...
        let res: Vec<RevealedFilter> = event.query().await?;

        for r in res.iter() {
            if r.sequence_number == sequence_number && r.provider == provider {
                tracing::info!(
