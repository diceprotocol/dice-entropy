...
### Keeper Enhancements
- [ ] Verify and document multi-replica Tyche failover (2+ keepers)
...
### SDK
- [x] npm publish `@diceprotocol/sdk`
...
### Contract
- [ ] Hash chain renewal automation (alert when 90% of the active chain is consumed)
...
### Infrastructure
- [ ] Dedicated infrastructure migration (isolation from other services)
...
- ❌ Slashing — relies on operational uptime, not economic penalties

## $DICE Token (planned)

The $DICE token is planned and not yet released. In v1, fees are paid in native ETH and the protocol operates without a token. Tokenomics, staking, burns, and governance are forward-looking design specifications, not live functionality.
